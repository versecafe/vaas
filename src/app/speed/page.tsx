...
const loadingStates = [
  ...
  {
    text: "Filtering For Speed Insights",
  },
];
      ...
      {teams[0] != undefined ? (
        <div className="p-4 max-w-7xl flex-1 justify-center mx-auto relative z-10 w-full pt-20 sm:pt-0">
          ...
          <p className="mt-4 font-normal text-base text-neutral-300 max-w-[600px] text-center mx-auto">
            Select a project to pull your speed insights data from. You can
            always come back and select a different project.
          </p>
          <ul className="pt-8">
            {teams.map((team) => (
              <li key={team.id}>
                <ul>
                  {team.projects.map((project) => (
                    ...
                    >
                      <Link href={`/speed/${token}/${team.id}/${project.id}`}>
                        {project.name}
                      </Link>
                    </li>
                  ))}
